...
def mnist_fnn(x_train, y_train, x_test, y_test):
    ...
    x = tf.keras.layers.Dense(
        32, activation='relu', kernel_initializer='glorot_uniform', bias_initializer='zeros')(x)
...
def mnist_cnn(x_train, y_train, x_test, y_test):
    ...
    inputs = tf.keras.Input(shape=(28, 28, 1), name='data')
    x = tf.keras.layers.Conv2D(2, kernel_size=(5, 5), strides=(
        1, 1), activation='relu', padding='same')(inputs)
    x = tf.keras.layers.MaxPooling2D(2, strides=(2, 2))(x)
    x = tf.keras.layers.Conv2D(2, kernel_size=(5, 5), strides=(
        1, 1), activation='relu', padding='valid')(x)
    x = tf.keras.layers.MaxPooling2D(2, strides=(2, 2))(x)
