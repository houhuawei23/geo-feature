...
def label_parallel(info_list, unit_hop, ratio, percentage, l, new_data_dict):
    ...
    weight = info_list[6]
    asub_data, add_data = dra.whole_augment(dataset, ave_egr, path_dict, gdist, edist, path_index, weight, unit_hop, ratio, percentage)
    new_data_dict [l] = asub_data
...
if __name__ == "__main__":
    ...
    for k in knn:        #iteration for knn
        ...
        for unit_hop in floatrange(hyper_uhop[0][1], hyper_uhop[0][2], 7)[0:-3]:
            for ratio in floatrange(hyper_ratio[0][2], hyper_ratio[0][1], 7)[1:-3]:
                for percentage in per:
                    ...
                    print("Rhyperparmater:knn-unit_hop-ratio-percentage-result(modified)", k, unit_hop, ratio, percentage, size2 - size1)
